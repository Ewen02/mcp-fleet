...
mcp-paie-fr is a stateless calculator.

- **What is sent**: the parameters of a calculation (gross salary, period, executive status, household situation…), chosen by the AI client from your conversation.
...
- **Log retention**: application logs are rotated (3 files of 10 MB). The reverse proxy keeps no access log.
- **Third parties**: none. Calculations run locally with the open-source URSSAF rules engine; no data is sent to URSSAF or any other service.
