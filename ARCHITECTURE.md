...
- **CI/CD**: lint (Biome), typecheck, tests, build and `npm audit` on every PR; on `main`, image pushed to GHCR (`sha-…` + `latest`) and deployed over SSH with `docker compose up --wait`, then an HTTPS smoke test. Deployment is opt-in (`DEPLOY_ENABLED`). *(Superseded by D24: `deploy.sh` with rollback.)*
