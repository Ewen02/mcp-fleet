...
- **Container**: multi-stage build, production dependencies only, non-root user, read-only filesystem, all capabilities dropped, memory/CPU/PID limits, healthcheck on `/health`, bound to `127.0.0.1` behind Caddy. Measured: ~55 MB RAM idle, stop in 0.2 s. *(Networking and deployment superseded by D24: no published port at all, central Caddy container.)*
- **Data minimization**: no Caddy access log (it would write client IPs to disk); the app logs requests without IP or body.
- **CI/CD**: lint (Biome), typecheck, tests, build and `npm audit` on every PR; on `main`, image pushed to GHCR (`sha-…` + `latest`) and deployed over SSH with `docker compose up --wait`, then an HTTPS smoke test. Deployment is opt-in (`DEPLOY_ENABLED`). *(Superseded by D24: per-server images, affected-only, `deploy.sh` with rollback.)*
...
- **`servers/<name>`** holds the domain, the tools, the domain tests and its deployment files. One server = one package = one image = one container.
...
- **Turborepo** runs `lint`, `typecheck`, `test`, `build` from the root in dependency order (`dependsOn: ["^build"]`: the kit is compiled before a server is type-checked, tested or built), caches results, and answers "what did this change affect?" (D24).
...
### D24 — One server, one deployable unit, on the VPS convention
The VPS convention (`~/infra/CLAUDE.md`) replaces the host-level Caddy and `127.0.0.1:3000` binding of D20:

- **Image per server**: one `Dockerfile` at the root, parameterized by `--build-arg SERVER=<folder>`. `pnpm fetch` only reads the lockfile (the download layer stays cached while code changes), then an offline install limited to `{./servers/<name>}...` (the server and its workspace dependencies: the kit), a build in dependency order and `pnpm deploy --prod` (a standalone folder with the compiled kit copied in). Runtime: uid 1001, no npm/corepack, files owned by root. No cache mount on purpose: GitHub's cache does not keep them, and the offline install would fail in CI.
- **On the VPS**: one project per server, named after its package (`mcp-paie-fr`): `~/apps/<name>/` (compose + `.env`), network `web-<name>`, **no published port**, the central Caddy container reaching it by name. The healthcheck probes every 2 s during start-up (`--start-interval`), so a deployment sees `healthy` in ~3 s instead of ~30 s.
- **Caddy snippet `(mcp)`**, next to the convention's `(app)`, differs on purpose: no access log (the app already logs without IP or body, PRIVACY.md), no compression (it can hold back an SSE stream), upstream keep-alive 30 s < Node's 65 s (Caddy's default is 2 min, and a POST is never retried: random 502s). The global error-log filter from D22 moves to the central Caddyfile. Verified with the real image behind Caddy 2.11.4: byte-identical results to v0.4.0 over HTTPS, no IP in Caddy's 502 logs, a forged `X-Forwarded-For` does not bypass the rate limit. In production the filter is a second line of defense: as soon as another site has a `log` directive, Caddy puts hosts without one (the MCP servers) in `skip_hosts` and writes no line for them at all, errors included (checked with a forced 502).
- **CI only rebuilds and redeploys what changed**: `turbo --affected` against the previous commit (push) or the target branch (PR). `scripts/affected-servers.mjs` turns Turborepo's answer into a job matrix: a server's files → that server; the kit → every server depending on it; `Dockerfile`, `.dockerignore`, `pnpm-workspace.yaml` (turbo `globalDependencies`) → all; root docs, workflows, Caddy fragment → none. Checked on a throwaway repository with a second server. Each image is smoke-tested with production hardening before being pushed.
- **Deployment through `deploy.sh`**: one deploy key per repository, restricted by `command=` to the list of this repository's projects. `deploy.sh` pulls the commit's image, waits for `healthy` and rolls back to the previous tag otherwise.
