/**
 * Point d'entrée distant d'un serveur : charge la configuration, démarre le
 * serveur HTTP, gère le cycle de vie du process. Tout le reste est dans
 * http-app.ts.
 *
 * Usage, dans le `src/http.ts` d'un serveur :  runHttp(definition)
 * Variables d'environnement : voir config.ts et DEPLOY.md.
 */
...
export function runHttp(definition: McpServerDefinition): void {
  ...
  const app = createHttpApp(definition, { ...config.app, logger })
